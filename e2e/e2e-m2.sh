...
# Clean DB
sudo -u postgres psql forge -c "TRUNCATE users CASCADE;" || true
...
NODE_COUNT=$(sudo -u postgres psql forge -t -c "SELECT COUNT(*) FROM nodes WHERE name='worker-1'")
if [ "$NODE_COUNT" -eq 0 ]; then
    echo "FAIL: node not registered in DB"
    kill $FORGED_PID $NODED_PID || true
    exit 1
fi
...
python3 ws_test.py "ws://localhost:8080/v1/projects/$PROJ_ID/stream" "forge_session=$COOKIE_VAL" || { echo "FAIL: WS events stream failed"; exit 1; }
